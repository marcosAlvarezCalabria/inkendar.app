...
begin
 ...
 update public.free_choice_pending_request r set status='EXPIRED' where r.studio_id=p_studio_id and r.status='PENDING_OWNER_APPROVAL' and r.expires_at<=now() and not exists(select 1 from public.free_choice_approval_operation o where o.request_id=r.id and o.state='INSERTING');
