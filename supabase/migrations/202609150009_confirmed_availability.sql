create index booking_option_confirmed_interval_idx
  on public.booking_option(studio_id, artist_profile_id, start_at, end_at)
    ...
    if exists(
      ...
      where option.studio_id = p_studio_id
        ...
        and (
          ...
          or (option.status = 'SELECTED' and offer.status = 'SELECTED_PENDING_CONFIRMATION' and offer.expires_at > p_now)
...
revoke all on function public.create_booking_offer(uuid,uuid,uuid,uuid,jsonb,timestamptz), public.list_active_booking_holds(uuid,uuid,uuid,timestamptz,timestamptz,timestamptz) from public, anon, authenticated;
grant execute on function public.create_booking_offer(uuid,uuid,uuid,uuid,jsonb,timestamptz), public.list_active_booking_holds(uuid,uuid,uuid,timestamptz,timestamptz,timestamptz) to service_role;
