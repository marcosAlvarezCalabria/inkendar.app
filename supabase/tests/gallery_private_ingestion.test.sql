...
select ok(has_function_privilege('authenticated','public.list_gallery_drafts(uuid,integer)','execute'),'authenticated may call identity-bound list');
select ok(not has_function_privilege('anon','public.list_gallery_drafts(uuid,integer)','execute'),'anonymous cannot list drafts');
...
select is((select count(*) from public.list_gallery_drafts('20000000-0000-0000-0000-000000000001',100)),2::bigint,'owner lists only own drafts');
select is((select string_agg(position::text,',' order by position) from public.list_gallery_drafts('20000000-0000-0000-0000-000000000001',100)),'1,1'::text,'positions are stable per target');
...
select throws_ok($$select * from public.list_gallery_drafts('20000000-0000-0000-0000-000000000001',101)$$,'22023','gallery draft invalid','list limit is bounded');

reset role; set local role authenticated;
select set_config('request.jwt.claim.sub','10000000-0000-0000-0000-000000000002',true);
select throws_ok($$select * from public.list_gallery_drafts('20000000-0000-0000-0000-000000000001',100)$$,'42501','gallery unavailable','artist cannot list');
...
select throws_ok($$select * from public.list_gallery_drafts('20000000-0000-0000-0000-000000000001',100)$$,'42501','gallery unavailable','cross-tenant owner cannot list');

reset role; set local role service_role;
select throws_ok($$select * from public.list_gallery_drafts('20000000-0000-0000-0000-000000000001',100)$$,'42501',null,'service role misuse fails closed');
