...
import type { ConversationSummary } from "@inkendar/application";
...
export default function OwnerInbox() {
  const data = useLoaderData() as { conversations: readonly ConversationSummary[]; error?: string };
  return <main className="shell-page">
    ...
    <section className="shell-panel" aria-labelledby="conversation-list-title">
      ...
      {data.conversations.length === 0 ? <p>Todavía no hay conversaciones disponibles.</p> : <ul className="conversation-list">
        {data.conversations.map((conversation) => <li key={conversation.id}>
