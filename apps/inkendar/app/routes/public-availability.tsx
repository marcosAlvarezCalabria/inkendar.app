...
export async function action({request,params}:Route.ActionArgs){const response=await publicFreeChoiceAvailabilityHandlers.action(request,params.token);if(!response.ok)throw new Response(response.status===503?"La disponibilidad no está disponible temporalmente.":"Este enlace no está disponible.",{status:response.status,headers:publicFreeChoiceAvailabilityHeaders()});return response;}
...
function content(data:PublicFreeChoiceAvailabilityView){
 if(data.state==="OPEN")return <><h1>Huecos disponibles</h1><p>Zona horaria: {data.timeZone}</p><p>Consulta válida hasta <time dateTime={data.expiresAt}>{data.expiresAt}</time>.</p><p>Estos huecos para {data.artistDisplayName} son orientativos y requieren aprobación del estudio.</p>{data.slots.length===0?<p>No hay huecos candidatos en este rango.</p>:<ol>{data.slots.map(slot=><li key={slot.selector}><Form method="post" reloadDocument><input type="hidden" name="selector" value={slot.selector}/><time dateTime={slot.startUtc}>{slot.startLocal}</time> – <time dateTime={slot.endUtc}>{slot.endLocal}</time> <button type="submit">Solicitar este hueco</button></Form></li>)}</ol>}</>;
 if(data.state==="PENDING_OWNER_APPROVAL")return <><h1>Solicitud recibida</h1><p>Tu elección está pendiente de aprobación del estudio. Todavía no es una cita confirmada.</p><Interval slot={data.selectedSlot}/></>;
 if(data.state==="APPROVING")return <><h1>Solicitud en proceso</h1><p>Estamos comprobando el resultado con el calendario. Todavía no podemos afirmar que exista una cita confirmada.</p></>;
 if(data.state==="CONFIRMED")return <><h1>Cita confirmada</h1><p>El estudio ha confirmado tu solicitud.</p><Interval slot={data.selectedSlot}/></>;
 if(data.state==="REJECTED")return <><h1>Solicitud no aceptada</h1><p>La solicitud no fue aceptada. Contacta con el estudio si necesitas otra opción.</p></>;
