  ...
  return <main className="shell-page">
    ...
    <section className="records" aria-labelledby="gallery-drafts-title"><h2 id="gallery-drafts-title">Borradores privados</h2>{data.drafts.length ? <ul className="gallery-grid">{data.drafts.map((draft) => <li className="shell-panel gallery-draft-card" key={draft.thumbnailHandle}>
      ...
      <form method="post" className="record-form" aria-label={`Editar ${draft.altText}`}>
        <input type="hidden" name="intent" value="UPDATE_DRAFT" /><input type="hidden" name="handle" value={draft.thumbnailHandle} />
      ...
      <form method="post" className="record-form"><input type="hidden" name="intent" value="DISCARD_DRAFT" /><input type="hidden" name="handle" value={draft.thumbnailHandle} /><p>Oculta este borrador de la lista y de sus miniaturas. Podrás recuperarlo más adelante.</p><button type="submit" className="secondary" aria-label={`Descartar borrador ${draft.altText}`}>Descartar borrador</button></form>
