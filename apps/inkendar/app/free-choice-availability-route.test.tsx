...
describe("public free-choice route",()=>{
 it("submits opaque candidate selectors through a canonical document request",async()=>{const html=await render({state:"OPEN",rangeStart:"2026-09-21T00:00:00.000Z",rangeEnd:"2026-09-22T00:00:00.000Z",durationMinutes:60,expiresAt:"2026-09-21T12:00:00.000Z",artistDisplayName:"Ana",timeZone:"Europe/Dublin",slots:[{selector:"ab".repeat(32),startUtc:"2026-09-21T09:00:00.000Z",endUtc:"2026-09-21T10:00:00.000Z",startLocal:"2026-09-21T10:00",endLocal:"2026-09-21T11:00"}]});expect(html).toContain("Huecos disponibles");expect(html).toContain("Solicitar este hueco");expect(html).toContain('data-submission-mode="document"');expect(html).not.toMatch(/customer|caseId|studioId|calendarId/iu);});
