...
| Operación dentro de Inkendar | `IN_PROGRESS` | Bandeja, mensajes, respuesta de texto, vínculo cliente/caso y webhook firmado están implementados detrás de puertos; `pnpm run check` pasó con 129 pruebas y 1 integración omitida. La migración y sus 32 aserciones pgTAP esperan ejecución real en CI porque Docker local no está disponible. |
...
| 2026-09-14 | DEC-027 | `ACCEPTED` | El primer corte de conversaciones OWNER consulta Chatwoot bajo demanda detrás de `ConversationProviderPort`, conserva solo vínculos e ingesta técnica en Supabase y autentica webhooks con firma HMAC, frescura y delivery ID idempotente. | Ocultar el proveedor, evitar copias divergentes de mensajes y conectar conversaciones con clientes/casos manteniendo aislamiento tenant y reintentos observables. |
