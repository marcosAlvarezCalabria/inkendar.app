...
| Hosting Cloudflare y salida a producción | `IN_PROGRESS` | El [PR #42](https://github.com/marcosAlvarezCalabria/inkendar.app/pull/42) está abierto. Su implementación de despliegue quedó fijada en `c2727ee`, con `validate` y `database` verdes en el [run 35879758897](https://github.com/marcosAlvarezCalabria/inkendar.app/actions/runs/35879758897); los commits posteriores son documentales y deben revalidarse antes del merge. Prepara `inkendar.calalva82.workers.dev` y staging, pero no se ha fusionado ni desplegado; tampoco existen todavía proyectos Supabase Cloud de Inkendar, secretos remotos, migraciones Cloud ni smoke externo. |
...
10. **Elección libre:** el cliente consulta candidatos mediante un enlace seguro ligado a su caso y puede seleccionar uno; Inkendar revalida FreeBusy y holds y crea una solicitud durable `PENDING_OWNER_APPROVAL`. La solicitud, aprobación/rechazo OWNER, Google Event recuperable y terminal seguro quedaron integrados mediante los PR #37 y #38. La notificación durable de rechazo está revisada en el PR #43 con CI verde; merge y prueba live siguen pendientes.
