...
| Google Calendar y booking | `IN_PROGRESS` | OAuth/asignación, disponibilidad, ofertas/holds, acceso, selección pública y confirmación recuperable están técnicamente `DONE`. La elección libre por caso y su solicitud durable `PENDING_OWNER_APPROVAL` quedaron integradas mediante el PR #37. Aprobación/rechazo OWNER, Google Event recuperable y terminal público son un candidato local verificado, pendiente de revisión, PR, CI y evidencia live; la notificación de rechazo sigue fuera. El 2026-09-16 una prueba live sintética verificó solo el recorrido preaprobado. Scheduler Chatwoot y fallback SMTP quedaron integrados mediante los PR #23 y #24 con CI verde; no existe prueba live de notificaciones. |
...
| 2026-09-18 | DEC-044 | `ACCEPTED` | Un token de elección libre consumido pierde autoridad de selección al vencer, pero conserva consulta terminal mínima mientras se retengan acceso y solicitud; no puede rotarse. La aprobación OWNER fija binding Google inmutable y una sola autoridad `READY → INSERTING`; desde `INSERTING` sólo reconcilia, sobrevive a caducidad y finaliza una única cita/relación. | Evitar reanunciar huecos, perder el resultado del cliente, duplicar eventos o liberar un intervalo ambiguo ante retries, respuesta perdida o carreras approve/reject/expiry. |
