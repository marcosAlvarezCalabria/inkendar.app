...
- **Google Calendar: IN_PROGRESS.** OAuth, listado y asignación live pasaron con owner sintético el 2026-09-15; la disponibilidad, el núcleo de ofertas y holds y su acceso público de solo lectura están técnicamente `DONE` con validación sintética y CI. Faltan FreeBusy live, eventos, selección pública, confirmación y booking extremo a extremo; no se ha ejecutado una prueba live de ofertas.
...
3. Implementar selección pública y confirmación idempotente contra Google en slices separados; el acceso público de solo lectura y el núcleo de ofertas preaprobadas, bloqueos y caducidad ya están integrados.
