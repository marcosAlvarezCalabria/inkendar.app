...
| Galería, portfolios y publicación web | `IN_PROGRESS` | Los PR #26–#30 integraron con CI verde ingestión/sanitización/proxy, curación OWNER, publicación/retirada durable, feed/API público read-only y web component framework-agnostic. La restauración OWNER de `DISCARDED` está `IN_PROGRESS` local. No existe evidencia live de Storage; faltan prueba en una web nueva y otra existente e invalidación CDN específica. GC sigue bloqueado hasta definir retención, grace period y reconciliación segura. |
...
| 2026-09-17 | DEC-041 | `ACCEPTED` | Un OWNER puede listar hasta 100 assets `DISCARDED` de su estudio mediante metadata editorial segura y restaurarlos por handle opaco con `RESTORE`. La transición `DISCARDED → DRAFT` conserva grupo, alt, variantes y binding, toma primero el lock común del estudio, anexa al máximo del grupo y converge sin mover de nuevo un asset ya `DRAFT`. | Recuperar trabajo privado ante descartes accidentales sin miniaturas de descartados, IDs internos, service role, Storage, hard delete ni reapertura de estados publicados. GC permanece bloqueado hasta acordar retención, grace period y reconciliación tras purga. |
...
And toma primero el lock común del estudio, cambia DISCARDED a DRAFT y anexa al máximo del mismo grupo
...
4. Revisar e integrar la restauración recuperable OWNER; ingestión, curación, publicación/retirada, feed/API y web component quedaron integrados mediante los PR #26–#30 con CI verde y sin prueba live de Storage.
