...
| Google Calendar y booking | `IN_PROGRESS` | OAuth/asignación, disponibilidad, ofertas/holds, acceso y selección públicos están técnicamente `DONE` en sus PR #10, #12, #14, #16 y #18. La confirmación recuperable está técnicamente `DONE` localmente: ID Google determinista, reconciliación `Events.get`, FreeBusy final, inserción privada/opaque sin PII, finalización Supabase atómica e idempotente y UI fail-closed; pasó 308 pruebas Vitest, build y 49 aserciones pgTAP nuevas dentro de 373. Revisión/integración siguen pendientes y no existe evidencia live de ofertas, FreeBusy, Google Events, confirmación ni booking extremo a extremo. |
...
| 2026-09-15 | DEC-033 | `ACCEPTED` | Una selección preaprobada se confirma con identidad Google SHA-256/base32hex determinista y correlación privada: cada intento reconcilia primero, solo un 404 permite FreeBusy final e insert, y una RPC serializada finaliza cita/relación/estados. La opción confirmada queda como exclusión local conservadora e inmutable, no como segunda agenda editable. | Evitar duplicados y confirmaciones falsas ante respuestas ambiguas, colisiones, cambios de asignación o retries concurrentes, manteniendo Google como agenda operativa. |
...
| 2026-09-15 | 1.12.0 | Se implementó localmente la confirmación recuperable de selección preaprobada con Google Events, FreeBusy final, ID determinista, relación/cita atómica, scope incremental, estados públicos seguros y exclusión confirmada; revisión, integración/CI y pruebas live continúan pendientes. | Completar el siguiente vertical técnico sin atribuir evidencia live ni anticipar notificaciones, scheduler, elección libre o cancelación. |
