...
| Operación dentro de Inkendar | `IN_PROGRESS` | Bandeja paginada, mensajes incrementales, respuesta idempotente, vínculo cliente/caso y webhook firmado están implementados detrás de puertos; 155 pruebas y el build pasan localmente. Las migraciones y pgTAP esperan ejecución real en CI porque Docker local no está disponible. |
