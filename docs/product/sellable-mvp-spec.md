...
| Operación dentro de Inkendar | `IN_PROGRESS` | Código listo: bandeja SSR OWNER, lectura bajo demanda, respuestas idempotentes y adaptadores Chatwoot/Supabase pasaron `npm run check` con 148 pruebas; faltan Postgres/pgTAP y el recorrido sintético real con Chatwoot en CI. |
...
| 2026-09-14 | DEC-026 | `ACCEPTED` | La bandeja owner lee Chatwoot bajo demanda y persiste solo conexiones, enlaces e intentos salientes opacos; una respuesta confirmada se reutiliza y un resultado ambiguo nunca se reenvía automáticamente. | Ocultar el proveedor y evitar mensajes duplicados sin copiar contenido ni secretos a Postgres o al navegador. |
...
|---|---|---|---|
| 2026-09-14 | 1.6.0 | Se implementó la bandeja SSR OWNER con frontera Chatwoot, metadata Supabase tenant-scoped y envío idempotente; las pruebas reales de Postgres y Chatwoot quedan pendientes. | Habilitar operación de mensajería dentro de Inkendar sin exponer proveedor, credenciales o contenido persistido y sin declarar cerrado el gate externo. |
