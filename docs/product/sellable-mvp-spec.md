...
| Galería, portfolios y publicación web | `IN_PROGRESS` | Los PR #26–#29 integraron con CI verde ingestión/sanitización/proxy, curación OWNER, publicación/retirada durable y feed/API público read-only; no existe evidencia live de Storage. El web component framework-agnostic está implementado localmente como asset ESM estable con Shadow DOM, estados accesibles, origen Inkendar seguro y personalización CSS acotada. Faltan revisión/CI del componente, prueba en una web nueva y otra existente, prueba live e invalidación CDN específica. |
