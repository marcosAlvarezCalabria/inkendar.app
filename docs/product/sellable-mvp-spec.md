...
| Hosting Cloudflare y salida a producción | `IN_PROGRESS` | El [PR #42](https://github.com/marcosAlvarezCalabria/inkendar.app/pull/42) quedó integrado como `271cb35`. Existe el proyecto Supabase Cloud `inkendar-staging` con migraciones aplicadas y el Worker `inkendar-staging`; el 2026-09-26 se configuraron sus secretos Supabase. Staging ejecuta `main` `62db889` como versión `47191259-514f-45b1-8559-f221e8825e97`; `/readyz` devuelve `200`, login OWNER y Ofertas pasaron smoke externo/móvil y el shell offline pasó el smoke automatizado parcial descrito en PWA. Producción no se ha desplegado ni configurado; Google, Chatwoot, Storage/Images y los recorridos completos de staging conservan sus gates propios. |
