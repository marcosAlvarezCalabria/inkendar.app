...
- **Google Calendar: IN_PROGRESS.** OAuth, listado y asignación live pasaron con owner sintético el 2026-09-15; la disponibilidad, el núcleo de ofertas y holds y su acceso público de solo lectura están técnicamente `DONE` con validación sintética y CI. La selección pública está implementada localmente y pendiente de revisión/CI; faltan FreeBusy live, Google Events, confirmación y booking extremo a extremo, y no se ha ejecutado una prueba live de ofertas.
...
3. Revisar e integrar la selección pública pendiente de confirmación y después implementar la confirmación idempotente contra Google en un slice separado; el acceso público de solo lectura y el núcleo de ofertas preaprobadas, bloqueos y caducidad ya están integrados.
