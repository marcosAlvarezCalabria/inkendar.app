...
| Google Calendar y booking | `IN_PROGRESS` | OAuth y asignación por artista están `DONE`: el [PR #10](https://github.com/marcosAlvarezCalabria/inkendar.app/pull/10) pasó 188 pruebas y 34 aserciones pgTAP del slice; OAuth, listado y asignación live pasaron localmente el 2026-09-15 con owner sintético. La disponibilidad técnica está `DONE`: el [PR #12](https://github.com/marcosAlvarezCalabria/inkendar.app/pull/12) pasó 215 pruebas, build, migraciones limpias y 27 aserciones pgTAP del slice dentro de 232 en el [run 34963263821](https://github.com/marcosAlvarezCalabria/inkendar.app/actions/runs/34963263821). El núcleo técnico de ofertas preaprobadas, holds y caducidad está `DONE`: el [PR #14](https://github.com/marcosAlvarezCalabria/inkendar.app/pull/14) pasó 230 pruebas, build, migraciones limpias y 29 aserciones pgTAP del slice dentro de 261; el CI posterior al merge quedó verde en el [run 34973774932](https://github.com/marcosAlvarezCalabria/inkendar.app/actions/runs/34973774932). El acceso público de solo lectura está `DONE`: el [PR #16](https://github.com/marcosAlvarezCalabria/inkendar.app/pull/16) pasó 259 pruebas, build, migraciones limpias y 27 aserciones pgTAP del slice dentro de 288; el CI posterior al merge quedó verde en el [run 34981646270](https://github.com/marcosAlvarezCalabria/inkendar.app/actions/runs/34981646270). La selección pública atómica está `DONE`: el [PR #18](https://github.com/marcosAlvarezCalabria/inkendar.app/pull/18) pasó 280 pruebas, build, migraciones limpias y 36 aserciones pgTAP del slice dentro de 324; el CI posterior al merge quedó verde en el [run 34996091108](https://github.com/marcosAlvarezCalabria/inkendar.app/actions/runs/34996091108). Conserva la opción elegida como hold y la comunica como pendiente de confirmación. No existe evidencia live de ofertas; FreeBusy live, Google Events, confirmación y booking extremo a extremo siguen `IN_PROGRESS`. |
...
- **Google Calendar: IN_PROGRESS.** OAuth, listado y asignación live pasaron con owner sintético el 2026-09-15; la disponibilidad, el núcleo de ofertas y holds, el acceso público y la selección pendiente de confirmación están técnicamente `DONE` con validación sintética y CI. Faltan FreeBusy live, Google Events, confirmación y booking extremo a extremo, y no se ha ejecutado una prueba live de ofertas.
...
3. Implementar la revalidación final y confirmación idempotente contra Google en un slice separado; la selección pública pendiente de confirmación, el acceso público de solo lectura y el núcleo de ofertas preaprobadas, bloqueos y caducidad ya están integrados.
