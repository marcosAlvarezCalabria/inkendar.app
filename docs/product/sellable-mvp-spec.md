...
| Operación dentro de Inkendar | `IN_PROGRESS` | Bandeja, mensajes, respuesta de texto, vínculo cliente/caso y webhook firmado están implementados detrás de puertos; `pnpm run check` pasó con 142 pruebas y 1 integración omitida. La migración y sus 34 aserciones pgTAP esperan ejecución real en CI porque Docker local no está disponible. |
