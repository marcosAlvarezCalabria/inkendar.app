...
_Estado técnico: scheduler Chatwoot y fallback SMTP integrados mediante los PR #23 y #24 con CI verde. La extensión `REJECTED` para elección libre es candidato local verificado; revisión, CI y toda prueba live de notificaciones permanecen pendientes._
...
Las confirmaciones, vencimientos y rechazos free-choice prefieren la única conversación Chatwoot original del caso y usan SMTP por estudio como fallback si existe email válido. Conservan `NO_ROUTE` cuando falta ruta/configuración y `UNKNOWN` ante ambigüedad; ningún estado se presenta como enviado sin confirmación del proveedor. La extensión de rechazo está verificada solo localmente y los recordatorios quedan para slices posteriores.
