...
_Estado técnico de emisión y solicitud pendiente: `DONE`; el PR #37 y su CI posterior al merge verificaron el corte. La decisión OWNER y confirmación free-choice son un candidato local verificado, pendiente de revisión, PR, CI y prueba live._
...
_Estado técnico: scheduler Chatwoot y fallback SMTP integrados mediante los PR #23 y #24 con CI verde. La extensión `REJECTED` para elección libre corresponde a `76f5c1c`, revisado en el PR #43 todavía abierto, con `validate` y `database` verdes en el run 36123518176; merge y toda prueba live de notificaciones permanecen pendientes._
...
Calcula opciones con jornada, duración, márgenes, zona horaria y ocupación real de Google Calendar. Expone candidatos de elección libre mediante un enlace hash-only por caso y acepta una selección pública same-origin que crea una solicitud pendiente durable, sin afirmar cita. Gestiona ofertas preaprobadas, opciones, reservas provisionales, caducidad, confirmación y liberación idempotente. La solicitud libre integrada por el PR #37 ya dispone de un candidato local para decisión OWNER, evento Google idempotente y terminal público seguro; revisión, CI y live siguen pendientes.
...
Las confirmaciones, vencimientos y rechazos free-choice prefieren la única conversación Chatwoot original del caso y usan SMTP por estudio como fallback si existe email válido. Conservan `NO_ROUTE` cuando falta ruta/configuración y `UNKNOWN` ante ambigüedad; ningún estado se presenta como enviado sin confirmación del proveedor. La extensión de rechazo está revisada en el PR #43 con CI verde y pendiente de merge y prueba live; los recordatorios quedan para slices posteriores.
