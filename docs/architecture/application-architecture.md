...
_Estado técnico: ingestión privada `DONE` mediante PR #26 y CI verde, sin prueba live; curación privada `IN_PROGRESS` local. Publicación, feed público, CDN y componente permanecen fuera de estos slices._
...
`gallery_asset` y `gallery_variant` usan FKs/checks compuestos, posiciones únicas por grupo y estados `DRAFT | DISCARDED`; no conceden acceso directo. Las RPC solo se conceden a `authenticated`, resuelven handle+`auth.uid()` y exigen OWNER coherente; `service_role` está revocado en metadata y queda limitado a la ingestión de objetos en el bucket privado `gallery-private`. El listado entrega un handle público aleatorio, nunca path o URL firmada. `/app/owner/gallery/thumbnails/:handle` vuelve a autorizar OWNER, resuelve solo assets `DRAFT` del tenant y hace un fetch server-side acotado de una firma de 30 segundos: origen/prefijo Supabase fijos, redirects prohibidos, `image/webp`, tamaño persistido máximo 10 MiB y timeout 3 segundos. El HTML contiene únicamente la ruta same-origin opaca y la respuesta usa `private, no-store` y `nosniff`.

La curación usa formularios SSR sin JavaScript obligatorio y un único `intent` con campos exactos. Editar normaliza alt y valida `GALLERY` sin artista o `ARTIST_PORTFOLIO` same-tenant; si cambia de grupo toma advisory locks ordenados y anexa al máximo existente. `MOVE_UP | MOVE_DOWN` toma el lock del grupo e intercambia con el DRAFT vecino mediante constraint diferida; en bordes no modifica nada. Se conservan huecos al reasignar o descartar. `DISCARD_DRAFT` cambia idempotentemente a `DISCARDED`, conserva fila/variantes/paths privados para futura restauración o GC y no llama a Storage. Listado, edición, reorder y resolver de miniatura excluyen descartados.
