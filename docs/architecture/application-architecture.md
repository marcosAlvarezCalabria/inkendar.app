...
Aplicación deriva de la opción un `eventId` SHA-256/base32hex y una correlación privada separados. Una RPC de claim bloquea la oferta y persiste antes de Google el tuple inmutable estudio/artista/oferta/opción/conexión/calendario/evento/correlación. Un lease `READY` puede recuperarse si vence; `beginInsert` realiza el CAS único `READY → INSERTING`. A partir de `INSERTING`, incluso tras crash o lease vencida, los siguientes workers reciben `RECONCILE_ONLY`: consultan `Events.get` pero nunca adquieren una segunda autoridad de insert.
