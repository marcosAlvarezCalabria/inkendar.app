...
`gallery_asset` y `gallery_variant` usan FKs/checks compuestos, posición estable por destino y estado único `DRAFT`; no conceden acceso directo. Las RPC solo se conceden a `authenticated`, resuelven `auth.uid()` y exigen OWNER coherente. `service_role` queda limitado al bucket privado `gallery-private`; firma thumbnails por 60 segundos server-side y las URLs nunca se persisten.
