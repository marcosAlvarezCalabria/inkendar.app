...
- Errores públicos tipados: `InvalidOnboardingInputError`, `DuplicateIdentityError`, `StudioNotFoundError`, `ProvisioningFailedError` y `ProvisioningCompensationFailedError`.
...
- GREEN enfocado: 21/21 pruebas de aplicación, compensación, adaptador y CLI pasaron.
- GREEN completo: `npm run check` pasó lint, tipos de todos los workspaces, 27/27 pruebas y build cliente/SSR.
- Persistencia preparada: `202609130002_manual_onboarding.sql` contiene dos operaciones transaccionales con roles fijos, `search_path` vacío y ejecución limitada a `service_role`; `manual_onboarding.test.sql` contiene 15 aserciones para grants, coherencia, duplicados, referencias y rollback.
- Limitación local: `supabase start` y `supabase status` quedaron bloqueados sin salida en este entorno y se interrumpieron tras esperas acotadas. `npm run db:test` queda pendiente del job reproducible `database`; por ello este contrato permanece `IN_PROGRESS`.
