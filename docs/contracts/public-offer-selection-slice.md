...
`list_active_booking_holds` incluye opciones `HELD` de ofertas `OPEN` vigentes y la opción `SELECTED` de ofertas `SELECTED_PENDING_CONFIRMATION` mientras estén vigentes. El slice posterior de confirmación extiende esa exclusión: si la operación ya alcanzó `INSERTING`, conserva el hold aun después de `expires_at`. `expire_booking_offers` y la preparación transaccional de `create_booking_offer` comparten la materialización: una selección vencida todavía provisional (`READY` o sin operación) pasa a oferta `EXPIRED` y opción `RELEASED`, pero una selección `INSERTING` cuyo efecto externo debe reconciliarse nunca se libera ni se reoferta.
