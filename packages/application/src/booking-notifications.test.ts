...
function claimed(eventType: "CONFIRMED" | "EXPIRED" = "CONFIRMED"): BookingNotificationClaim {
  return { kind: "CLAIMED", jobId, leaseId, studioId, eventType, attemptCount: 1, externalAccountId: "3", externalConversationId: "42" };
}

function dependencies(claims: BookingNotificationClaim[] = [claimed(), { kind: "EMPTY" }]) {
  ...
  return { repository, provider, providerFor };
}
...
describe("booking notification runner", () => {
  ...
  it("generates the expiry copy only in memory", async () => {
    const deps = dependencies([claimed("EXPIRED"), { kind: "EMPTY" }]);
    const runner = createBookingNotificationRunner({ ...deps, clock: () => now, monotonicClock: () => 100 });
