...
import { FreeChoiceAvailabilityContextNotFoundError,type FreeChoiceAvailabilityAccessRepositoryPort,type FreeChoiceOwnerManagement,type PublicFreeChoiceAvailabilityContext,type PublicFreeChoiceAvailabilityRepositoryPort,type RotateFreeChoiceAvailabilityAccessRecord,type FreeChoicePendingRequest } from "@inkendar/application";
import { normalizePublicBookingOfferHash } from "@inkendar/domain";

type Result=Readonly<{data:unknown;error:unknown}>;
export interface FreeChoiceAvailabilityDataGateway { rotateAccess(parameters:Record<string,unknown>):Promise<Result>;getManagement(parameters:Record<string,unknown>):Promise<Result>; getPublicContext(parameters:Record<string,unknown>):Promise<Result>;selectPending(parameters:Record<string,unknown>):Promise<Result>;markReauthRequired(parameters:Record<string,unknown>):Promise<Result>; }
...
export class SupabaseFreeChoiceAvailabilityAccessRepository implements FreeChoiceAvailabilityAccessRepositoryPort {
  ...
  async rotateAccess(input:RotateFreeChoiceAvailabilityAccessRecord):Promise<Readonly<{expiresAt:string}>>{
    const result=await this.data.rotateAccess({p_studio_id:input.studioId,p_owner_user_id:this.ownerUserId,p_tattoo_case_id:input.tattooCaseId,p_artist_profile_id:input.artistProfileId,p_token_hash:normalizePublicBookingOfferHash(input.tokenHash),p_range_start:input.rangeStart,p_range_end:input.rangeEnd,p_duration_minutes:input.durationMinutes,p_expires_at:input.expiresAt,p_now:input.nowUtc});
    if(result.error||result.data===null)ownerFailed(result.error);
    return {expiresAt:timestamp(object(result.data).expires_at)};
  }
  async getManagement(studioId:string):Promise<FreeChoiceOwnerManagement>{const result=await this.data.getManagement({p_studio_id:studioId,p_owner_user_id:this.ownerUserId});if(result.error||result.data===null)ownerFailed(result.error);const row=object(result.data);return {cases:array(row.cases).map(value=>{const item=object(value);return {id:bounded(item.id,64),summary:bounded(item.summary,500),artistProfileId:bounded(item.artist_profile_id,64)};}),pendingRequests:array(row.pending_requests).map(value=>{const item=object(value),status=item.status;if(status!=="PENDING_OWNER_APPROVAL"&&status!=="APPROVING")failed();return {id:bounded(item.id,64),status,customerName:bounded(item.customer_name,200),caseSummary:bounded(item.case_summary,500),artistDisplayName:bounded(item.artist_display_name,120),startUtc:timestamp(item.start_at),endUtc:timestamp(item.end_at),expiresAt:timestamp(item.expires_at)};})};}
}
...
export class SupabasePublicFreeChoiceAvailabilityRepository implements PublicFreeChoiceAvailabilityRepositoryPort {
  ...
  async selectPending(input:Readonly<{tokenHash:string;selector:string;startUtc:string;endUtc:string;nowUtc:string}>):Promise<FreeChoicePendingRequest>{const result=await this.data.selectPending({p_token_hash:normalizePublicBookingOfferHash(input.tokenHash),p_selector:normalizePublicBookingOfferHash(input.selector),p_start_at:input.startUtc,p_end_at:input.endUtc,p_now:input.nowUtc});if(result.error||result.data===null)failed();return pending(object(result.data));}
}

export class SupabaseFreeChoiceAvailabilityGateway implements FreeChoiceAvailabilityDataGateway { constructor(private readonly rpc:(name:string,parameters:Record<string,unknown>)=>Promise<Result>){} rotateAccess=(parameters:Record<string,unknown>)=>this.rpc("rotate_free_choice_availability_access",parameters);getManagement=(parameters:Record<string,unknown>)=>this.rpc("get_free_choice_availability_management",parameters);getPublicContext=(parameters:Record<string,unknown>)=>this.rpc("get_public_free_choice_availability_context",parameters);selectPending=(parameters:Record<string,unknown>)=>this.rpc("select_public_free_choice_availability",parameters);markReauthRequired=(parameters:Record<string,unknown>)=>this.rpc("mark_public_free_choice_availability_reauth_required",parameters); }
...
function pending(row:Record<string,unknown>):FreeChoicePendingRequest{return {state:"PENDING_OWNER_APPROVAL",startUtc:timestamp(row.start_at),endUtc:timestamp(row.end_at),expiresAt:timestamp(row.expires_at)};}
