...
function validateInput(input:Readonly<{calendarId:string;timeMin:string;timeMax:string}>):void { if(input.calendarId.length<1||input.calendarId.length>1024||Array.from(input.calendarId).some((character)=>character.charCodeAt(0)<32||character.charCodeAt(0)===127))fail();const start=parseRfc3339(input.timeMin,true),end=parseRfc3339(input.timeMax,true);if(end<=start||end-start>31*86_400_000)fail(); }
